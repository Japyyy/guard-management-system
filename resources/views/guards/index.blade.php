...
@section('content')
    <div class="flex flex-col gap-5">
        <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
                <h1 class="text-2xl font-bold text-slate-900">Guard Records</h1>
                <p class="mt-1 text-sm text-slate-500">Manage personnel profiles, license validity, and deployment assignments.</p>
            </div>

            <a href="{{ route('guards.create') }}"
               class="inline-flex items-center rounded-2xl bg-gradient-to-r from-blue-600 to-cyan-500 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-blue-500/20 hover:opacity-95">
                + Add Guard
            </a>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                    ...
                    <input type="text" name="search" value="{{ request('search') }}"
                           ...
                           class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100 outline-none">
                </div>
                ...
                <div>
                    <label class="mb-2 block text-xs font-semibold uppercase tracking-wide text-slate-500">Company</label>
        ...
        <div class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
            <div class="border-b border-slate-200 px-6 py-5">
                <h2 class="text-lg font-semibold text-slate-900">Personnel List</h2>
                <p class="mt-1 text-sm text-slate-500">Complete list of all security guards in the system.</p>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y divide-slate-200">
                            ...
                            <tr class="hover:bg-slate-50/80 transition">
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    @elseif($guard->status === 'Expiring in 30 Days')
                                        <span class="inline-flex rounded-full bg-amber-100 px-3 py-1 text-xs font-semibold text-amber-700">30 Days</span>
                                    @else
                                        <span class="inline-flex rounded-full bg-yellow-100 px-3 py-1 text-xs font-semibold text-yellow-700">60 Days</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center text-slate-500">No guards found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
